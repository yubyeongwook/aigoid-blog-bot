...
# ==========================================
# 1. AWS Lightsail WordPress 설정
# ==========================================
WP_URL = "http://43.200.245.223"
WP_USER = "user"
WP_PASS = "LaborCheck123!"

# 노무체크 AI Vercel 24시간 실시간 서비스 주소
...
def post_to_wordpress(keyword_info):
    html_body = generate_labor_post_html(keyword_info)
    endpoint = f"{WP_URL}/index.php?rest_route=/wp/v2/posts"
    ...
    try:
        res = requests.post(endpoint, json=payload, auth=auth, headers=headers, timeout=15)
        if res.status_code in [200, 201]:
            data = res.json()
            print(f"[SUCCESS] Post published! Post ID: {data.get('id')} / URL: {data.get('link')}")
            return True, data.get('link')
        else:
            print(f"[FAIL] Publish failed (Status Code {res.status_code}): {res.text}")
            return False, None
...
if __name__ == "__main__":
    print("[START] LaborCheck AI WordPress Auto Publisher Running...")
    target_kw = random.choice(LABOR_KEYWORDS)
    print(f"[TARGET] Keyword Title: {target_kw['title']}")
    success, post_url = post_to_wordpress(target_kw)
    if success:
        print(f"[COMPLETED] Post published successfully: {post_url}")
