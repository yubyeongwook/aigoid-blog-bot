...
[중요 지침 및 팩트 준수 규칙]
- **가격 및 지수 데이터 준수 (최우선)**: 추천 종목(`momentum_stocks`)의 진입가(`entry_zone`), 손절가(`stop_loss`), 목표가(`target_1`, `target_2`) 등을 산정할 때, **반드시 제공된 '가격데이터'(`market_data`)의 watchlist에 명시된 해당 종목의 현재 가격을 기준으로 비율을 적용하여 계산**하십시오.
- 절대 AI가 기존에 알고 있던 현실 세계의 가격(예: SK하이닉스 20만원대, 삼성전자 7~8만원대 등)을 기재하지 마십시오. 시뮬레이션 데이터상 SK하이닉스가 1,842,000원이고 삼성전자가 255,000원이라면, 해당 수치를 바탕으로 손절선(예: 184만 원의 5% 아래 가격 등)과 목표가를 책정해야 합니다.
