...
[최우선 지침 — 글로벌 매크로 → 한국 시장 연결 구조 필수]
제공된 market_data에는 반드시 `global_macro` 키가 포함되어 있습니다. 이 글로벌 데이터를 **분석의 출발점**으로 삼아야 합니다.
- `global_macro.nasdaq` / `global_macro.sp500` / `global_macro.soxx` → 미국 증시 및 반도체 섹터 동향
- `global_macro.dxy` → 달러 강약 → 원달러 환율 → 외국인 수급 영향
- `global_macro.us10y` / `global_macro.us2y` → 미 국채 금리 커브 → 성장주/가치주 밸류에이션 영향
- `global_macro.wti` / `global_macro.gold` → 인플레이션·지정학적 리스크 바로미터
- `global_macro.vix` → 글로벌 공포지수 → 외국인 리스크 온/오프 판단

이 지표들과 한국의 KOSPI·수급 데이터를 직접 연결하여 "글로벌 자금이 왜 한국으로 오거나 빠지는지"를 인과관계로 설명하십시오.
...
JSON Schema:
{
  "key_insight": "오늘 글로벌 시장(나스닥·달러·금리·유가)에서 관측된 가장 중요하고 반직관적인 매크로 팩트와 한국 시장에 대한 구조적 함의. 반드시 global_macro의 실제 수치를 포함할 것.",
  "dollar_rate_oil": {"current_structure": "DXY 달러인덱스, 미국채 10년 금리(^TNX), WTI 유가의 현재 삼각 상관관계 구조 분석 (실제 수치 명시)", "korea_impact": "위 지표들의 움직임이 원달러 환율과 국고채 금리를 통해 국내 증시 수급과 지수에 미치는 영향"},
  "us_market_korea_link": {"us_close": "전일 나스닥·S&P500·SOXX 반도체 ETF 실제 등락률과 한국 반도체주(삼성전자·SK하이닉스)에 미치는 당일 선행 영향 분석", "vix_signal": "VIX 공포지수 현황과 외국인 리스크 온/오프 판단"},
