...
6개 분야 수석 분석가(글로벌 매크로, 미국 야간 시황, 국내 수급, 실적/공시, 장전 거래량/인기 종목, 기술적 지표)의 분석을 종합하여, 세계 최고 헤지펀드 전략 리포트 수준의 독창적이고 심도 있는 블로그 포스트의 [전반부 (Part 1)]를 HTML 형식으로 작성합니다.

[중요 팩트 고수 및 분량 가이드]
- **시장 수치 절대 왜곡 금지 (필수)**: 제공된 원본 데이터(`market_data`)의 코스피(KOSPI), 코스닥(KOSDAQ) 지수 및 종목의 가격, 변동률 수치는 소수점까지 한 글자도 바꾸지 말고 **반드시 제공된 팩트 그대로 표기**하십시오. 흥미 위주로 지수가 9000선을 뚫었다거나, 6000선으로 대추락했다는 등의 가상 시나리오는 내용에 절대 작성하지 마십시오.
- 절대 느낌표(!), 급등, 대박 등의 자극적이거나 투자 유인/선동적인 어조를 사용하지 마십시오.
...
2. <style> 블록 포함.
3. 마스트헤드 테이블 (발행 정보)
4. 수치 대시보드 (차콜 배경 #0f172a 카드 및 수치들 - 팩트 수치 그대로 대입)
5. 히어로 이미지 (Unsplash 이미지 링크, 780px 너비)
6. H1 메인 헤드라인 (전일 종가 분석 결과와 오늘 장전 상황을 엮어낸 깊고 매력적인 역설형 제목. 수치는 팩트 그대로)
...
- **수치 고수 원칙**: 추천 종목에 매수 진입가, 목표가, 손절선 가격을 기입할 때, 원본 데이터의 가격 범위 내에서 합리적이고 정확한 숫자를 원 단위로 표기하십시오. 임의의 가격 왜곡을 금지합니다.
