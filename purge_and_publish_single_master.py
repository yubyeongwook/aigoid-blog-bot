...
def purge_all_old_posts():
    print("[PURGE] Purging all old posts via REST API...")
    auth = HTTPBasicAuth(WP_USER, WP_PASS)
    # 기존 REST API 게시물 목록 가져와서 전량 싹 삭제
    res = requests.get(f"{WP_BASE}/index.php?rest_route=/wp/v2/posts&per_page=100", auth=auth)
    if res.status_code == 200:
        posts = res.json()
        for p in posts:
            post_id = p['id']
            del_res = requests.delete(f"{WP_BASE}/index.php?rest_route=/wp/v2/posts/{post_id}&force=true", auth=auth)
            if del_res.status_code in [200, 201]:
                print(f"  └ [PURGED] Post ID {post_id} deleted permanently.")
    print("[PURGE COMPLETE] All previous posts cleaned up.")

def publish_single_master_post():
    date_str = datetime.now().strftime("%Y년 %m월 %d일")
    
    # 파이썬 동적 수치 정밀 계산 검증 (오차 0원)
    h2025 = 10030
...
if __name__ == "__main__":
    purge_all_old_posts()
    final_url = publish_single_master_post()
    print(f"[SUCCESS COMPLETE] Master Post Available At: {final_url}")
