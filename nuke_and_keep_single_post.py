...
def nuke_all_posts_and_pages():
    print("[NUKE] Nuking all existing posts, drafts, and trashed pages...")
    auth = HTTPBasicAuth(WP_USER, WP_PASS)
    
    # 1. 모든 포스트 상태(publish, draft, trash, future) 100% 소탕
    ...
    for st in statuses:
        ...
        if res.status_code == 200:
            ...
            for p in posts:
                p_id = p['id']
                del_res = requests.delete(f"{WP_BASE}/index.php?rest_route=/wp/v2/posts/{p_id}&force=true", auth=auth)
                print(f"  └ [NUKED POST] Post ID {p_id} ({st}) permanently deleted.")
                
    print("[NUKE COMPLETE] Every single old post has been completely wiped.")

def publish_the_only_master_post():
    date_str = datetime.now().strftime("%Y년 %m월 %d일")
    
    # 100% 파이썬 동적 수치 정밀 계산 검증
    h2025 = 10030
    m2025 = h2025 * 209
    h2026 = 10320
    m2026 = h2026 * 209
...
"""
    ...
    print(f"[THE ONLY SINGLE MASTER POST] ID: {post_id} / URL: {post_url}")
    return post_url

if __name__ == "__main__":
    nuke_all_posts_and_pages()
    final_url = publish_the_only_master_post()
    print(f"[NUKED & REBUILT SUCCESS] Only 1 Post Available: {final_url}")
