...
# 사장님이 선택하신 실시간 트래픽 폭발 2번~5번 주제 DB
TOPICS_2_TO_5 = [
    {
        "id": 2,
        "category": "산재보상",
...
def publish_prompt_applied_post(topic_data):
...
<div style="background:#0f172a;border:1px solid #334155;padding:26px;border-radius:12px;margin:36px 0;">
  ...
  <p style="font-size:15px;color:#e2e8f0;line-height:1.85;margin:0;">
    "노무 분쟁은 객관적 서면 자료와 법 조문의 정밀한 매칭이 핵심입니다. 감정적 대응보다는 출퇴근 기록, 임금 내역서 등 객관적 증거를 기반으로 전문가 및 AI 진단 엔진을 활용하는 것이 가장 안전합니다."
  </p>
</div>
  ...
  <tr>
    <td style="background:#f0f4ff;padding:16px 18px;">
      <p style="font-size:13.5px;color:#1e293b;line-height:1.8;margin:0;">
        본 글은 일반적인 인사노무 정보 제공 목적이며 구체적인 법적 조언이 아닙니다. 개별 사안에 따라 판단이 달라질 수 있으므로 구체적인 상담은 고용노동부(1350)에 문의하시기 바랍니다.
      </p>
    </td>
  </tr>
</table>
...
"""
    ...
    print(f"[SUCCESS PUBLISHED] Topic #{topic_data['id']} Post ID: {post_id} / URL: {post_url}")
    return post_url

if __name__ == "__main__":
    print("[PUBLISHING 2 TO 5] Starting publishing Topics #2 to #5 with User System Prompt...")
    for t in TOPICS_2_TO_5:
        publish_prompt_applied_post(t)
        time.sleep(1)
    print("[ALL DONE] Topics #2 to #5 successfully published to WordPress!")
