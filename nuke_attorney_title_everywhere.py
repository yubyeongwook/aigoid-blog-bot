...
# 1. 파이썬 코드 파일 전체에서 '노무체크 AI' 단어를 '노무체크 AI'로 전량 영구 치환
def clean_python_files():
    print("[CLEANING CODEBASE] Removing '노무체크 AI' from all python scripts...")
    py_files = glob.glob("c:/Users/aigoi/OneDrive/바탕 화면/안티 프로젝트/BLOG_AUTO/aigoid-blog-bot/*.py")
    for fpath in py_files:
        try:
            with open(fpath, 'r', encoding='utf-8') as f:
                content = f.read()
            if '노무체크 AI' in content:
                new_content = content.replace('노무체크 AI 정밀 총평', '노무체크 AI 정밀 총평')
                new_content = new_content.replace('노무체크 AI 정밀 총평', '노무체크 AI 정밀 총평')
                new_content = new_content.replace('노무체크 AI 인사이트', '노무체크 AI 인사이트')
                new_content = new_content.replace('', '')
                new_content = new_content.replace('노무체크 AI 레벨', '노무체크 AI 레벨')
                new_content = new_content.replace('노무체크 AI', '노무체크 AI')
...
# 2. 5대 핵심 포스팅 데이터 (노무체크 AI 단어 100% 완전 소탕)
ALL_5_CLEAN_TOPICS = [
    {
        "id": 1,
        "category": "임금·퇴직금",
        ...
        "check_3": "□ 임금체불 발생 시 고용노동부 1350 또는 노동포털 진정서 제출"
    },
    {
        "id": 2,
        "category": "산재보상",
...
def purge_all_wp_posts():
    print("[WP PURGE] Cleaning all old posts from WordPress database...")
    ...
    print("[WP PURGE COMPLETE] All WP posts cleaned.")

def publish_100percent_clean_post(topic_data):
...
"""
    ...
    print(f"[100% CLEAN POST PUBLISHED] Topic #{topic_data['id']} Post ID: {post_id} / URL: {post_url}")
    return post_url

if __name__ == "__main__":
    clean_python_files()
    purge_all_wp_posts()
    for t in ALL_5_CLEAN_TOPICS:
        publish_100percent_clean_post(t)
        time.sleep(1)
    print("[FINAL SUCCESS] '노무체크 AI' 100% completely purged from code & WordPress database!")
